...
export class MariadbPersonRepository implements PersonRepository {
    async disapprovedPerson(id_user: number): Promise<boolean | Error | null> {
        try {
            ...
            const updateQuery = "UPDATE person SET status = 'Disapproved' WHERE id_user = ?";
            const updateResult = await query(updateQuery, [id_user]);
    ...
    async validatePerson(id_user: number): Promise<boolean | null | Error> {
        try {
            const personQuery = "SELECT * FROM person WHERE id_user = ?";
            const personResult = await query(personQuery, [id_user]);
            console.log('personResult', personResult)
            ...
            const updateQuery = "UPDATE person SET status = 'Approved' WHERE id_user = ?";
            const updateResult = await query(updateQuery, [id_user]);
