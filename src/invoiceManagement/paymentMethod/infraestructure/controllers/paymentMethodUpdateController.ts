...
export class PaymentMethodUpdateController {
  ...
  async updatePayments(request: Request, response: Response) {
    try {
      ...
      const { name, status, pb_key_prod, pd_key_prod, pb_key_test, pd_key_test } = request.body;
      const updatedPaymentData = {
        id, name, status, pb_key_prod, pd_key_prod, pb_key_test, pd_key_test
      };

    
      const updatedPayment = await this.paymentsUpdateUseCase.execute(Number(id), updatedPaymentData);
